/*
     ...
     SELECT  (조회용쿼리문) 을 저장해둘 수 있는 표 형태의 객체
     ( 자주 쓰일법한 긴 SELECT 문을 저장해두면 그 긴 SELECT 문을 매번 작성해서 다시 조회할 필요가 없음)
...
WHERE E.DEPT_CODE= D.DEPT_ID(+)     --연결고리에 대한 조건
...
/*
      ...
      AS (서브쿼리);
      => 뷰 생성시 기존에 중복된 이름의 뷰가 없다면 새로이 뷰를 추가하고
                         기존에 중복된 이름의 뷰가 있다면 해당 뷰를 변경 하는 옵션 (OR REPLACE)      
*/

-- [ 참고 ] => 인라인 뷰 (71번째줄인 SELECT * FROM VW_EMPLOYEE; 와 같다.  )
SELECT * 
...
AS (SELECT EMP_ID, EMP_NAME  ,DEPT_TITLE , SALARY , NATIONAL_NAME ,  JOB_NAME 
      ...
      WHERE E.DEPT_CODE= D.DEPT_ID(+)     
          AND D.LOCATION_ID=L.LOCAL_CODE(+) 
          AND L.NATIONAL_CODE=N.NATIONAL_CODE(+)
          AND E.JOB_CODE=J.JOB_CODE );   --서브쿼리 부분에 '한국'에서 근무하는 조건은 제외
--insufficient privileges 
-- 현재 KH 계정에 CREATE VIEW 권한이 없어서 오류 발생
-- 해결방법 : 관리자계정으로 접속해서 GRANT CREATE VIEW TO KH;
...
-- 한번 서브쿼리로 뷰를 생성 후 해당 뷰명으로 SELECT 문을 이용하여 간단하게 조회 가능하다.
...
-- ( 뷰의 정의 자체가 SELECT문을 저장 == 단순히 쿼리문이 TEXT문구로 저장되어있음)
--  USER_VIEWS 데이터 딕셔너리 : 해당 계정이 가지고 있는 VIEW 들에 대한 정보들을 가지고 있는 관리용 테이블
SELECT *
FROM USER_VIEWS;
...
/*
    *  뷰 컬럼에 별칭 부여
    서브쿼리의 SELECT 절에 함수나 산술연산이 기술되어있는 경우 반드시 별칭 지정
*/
...
/*
    * 생성된 뷰를 이용해서 DML ( INSERT, UPDATE, DELETE) 사용 가능
      단, 뷰를 통해서 변경하게 되면 실제 원래 데이터가 담겨있던
      실질적인 테이블(베이스테이블) 에도 적용이 된다. 
*/
...
--베이스테이블 (JOB) 에도 값이 INSERT 된 것을 확인 가능

--뷰에 UPDATE
--JOB_CODE가 J8인 JOB_NAME 을 알바로 UPDATE
UPDATE  VW_JOB
      SET  JOB_NAME = '알바'
      WHERE JOB_CODE = 'J8';

--베이스테이블 (JOB)에도 값이 UPDATE 된것을 확인 가능

--뷰에 DELETE
--JOB_CODE가 J8인 행을 삭제
DELETE FROM VW_JOB
WHERE JOB_CODE = 'J8';
 --베이스테이블 (JOB) 에도 값이 DELETE 된것을 확인 가능 
...
--INSERT (오류)
INSERT INTO VW_JOB VALUES ('인턴');  --JOB 테이블에 (NULL , '인턴') 삽입되려고 함
--cannot insert NULL into (NULL 값이 들어갈수 없다.)=>JOB_CODE가 NULL 로 되게 만들려고하니까 
...
-- UPDATE (오류)
UPDATE VW_JOB
      SET JOB_CODE = NULL --> NOT NULL에 위배됨
WHERE JOB_NAME = '알바';  
--> VW_JOB에 JOB_CODE 컬럼이 없을 뿐만 아니라 NOT NULL제약조건인데 NULL을 넣고자해서 오류
...
WHERE JOB_NAME = '알바';
--child record found 
--만약에 J8 직급코드였다면 자식데이터가 없기 때문에 삭제가 가능했을것임!
...
WHERE 연봉 = 72000000;  
-- 원봉테이블에 실제 없는 컬럼으로 조건을 지어줬지만
-- 그 조건을 따라서 삭제를 하려고 봤더니 원본에 지울수 있는 값이 존재하기 때문에 실제로 삭제가 성공됨
...
WHERE JOB_CODE= 'J7';
 ...
 --cannot modify more than 여러개의 테이블이 조인된 뷰는 INSERT가 되지 않음

 --UPDATE (성공)
 UPDATE VW_JOINEMP
        SET EMP_NAME= '서동일'
  WHERE EMP_ID= 200;   
 --EMPLOYEE 테이블에만 반영 가능한  UPDATE문이라 수행 성공 ( 특이케이스)
 ...
 SELECT * FROM  VW_JOINEMP;
 SELECT * FROM EMPLOYEE;  -- 기준테이블 기준으로 DML 적용됨
...
/*
    * VIEW 옵션
    ...
    1) FORCE / NOFORCE
      ...
      - NOFORCE : 서브쿼리에 기술된 테이블이 반드시 존재해야만 뷰를 생성하겠다.  (생략시 기본값)
      
    2) WITH CHECK OPTION : 서브쿼리의 조건절에 기술된 내용에 만족하는 값으로만 DML 이 가능
...
--단, 해당 TT라는 이름의 테이블이 생성된 이후부터는 해당 뷰 사용 가능
CREATE TABLE TT (
     TCODE NUMBER,
     TNAME VARCHAR2(30),
...
SELECT * FROM VW_EMP;
-- 현재 이 뷰의 상황 : 월급이 300만원 이상인 사원들의 정보만 보임

UPDATE VW_EMP
     SET  SALARY = 2000000
WHERE EMP_ID = 200; 
--view WITH CHECK OPTION where-clause violation
--서브쿼리에 기술한 조건에 부합하지 않게끔 수정을 시도했기 떄문에 변경 불가
UPDATE VW_EMP
     SET  SALARY = 4000000
WHERE EMP_ID = 200; 
--서브쿼리에 기술한 조건에 부합해서 수정이 가능

SELECT * FROM EMPLOYEE;  --원본도 바뀐다.
